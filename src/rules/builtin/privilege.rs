...
fn pe_002() -> Rule {
    Rule {
        ...
        patterns: vec![
            ...
            Regex::new(r"\brm\s+((?:-[rfRF]+|--recursive|--force)\s+)+/[^a-zA-Z]")
                .expect("PE-002: invalid regex"),
            ...
            Regex::new(r"\brm\s+((?:-[rfRF]+|--recursive|--force)\s+)+\$\{?[A-Za-z_][A-Za-z0-9_]*\}?\s*$")
                .expect("PE-002: invalid regex"),
            ...
            Regex::new(r"\brm\s+((?:-[rfRF]+|--recursive|--force)\s+)+/(bin|boot|dev|etc|lib|lib64|opt|proc|root|sbin|sys|usr|var)\b")
                .expect("PE-002: invalid regex"),
        ],
        exclusions: vec![],
...
mod tests {
    ...
    fn test_pe_002_detects_rm_rf_root() {
        ...
        let test_cases = vec![
            ...
            ("rm -r --force /var", true),
            ("rm --recursive -f /", true),
            ("rm --recursive --force /; echo done", true),
            (r"rm --recursive --force /\*", true),
            ("rm --recursive --force $TARGET", true),
            ("rm --recursive --force $(printf /)", true),
            (r"rm --recursive --force `printf /`", true),
            ("rm -rf /tmp/test", false),
            ("rm --recursive --force /tmp/specific-dir", false),
