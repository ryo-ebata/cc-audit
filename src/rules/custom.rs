...
impl CustomRuleLoader {
    ...
    pub fn load_from_string(content: &str) -> Result<Vec<DynamicRule>, CustomRuleError> {
        let config: CustomRulesConfig =
            serde_norway::from_str(content).map_err(CustomRuleError::ParseError)?;
...
"#;
    ...
    fn test_parse_error_display() {
        ...
        let result: Result<CustomRulesConfig, _> = serde_norway::from_str(invalid_yaml);
